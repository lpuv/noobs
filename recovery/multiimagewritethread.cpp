...
MultiImageWriteThread::MultiImageWriteThread(const QString &bootdrive,
                                             const QString &rootdrive,
                                             QObject *parent)
    : QThread(parent), _drive(rootdrive), _bootdrive(bootdrive),
      _extraSpacePerPartition(0), _part(5) {
  QDir dir;
...
void MultiImageWriteThread::run() {
  ...
  uint totalnominalsize = 0, totaluncompressedsize = 0, numparts = 0,
       numexpandparts = 0;
  uint startSector =
  ...
  foreach (OsInfo *image, _images) {
    ...
    if (nameMatchesRiscOS(image->folder())) {
      /* Check the riscos_offset in os.json matches what we're expecting.
         In theory we shouldn't hit either of these errors because the invalid
         RISC_OS
         should have been filtered out already (not added to OS-list) in
         mainwindow.cpp */
      if (image->riscosOffset()) {
        if (image->riscosOffset() != RISCOS_OFFSET) {
          emit error(
              tr("RISCOS cannot be installed. RISCOS offset value mismatch."));
          return;
        }
      } else {
        emit error(
            tr("RISCOS cannot be installed. RISCOS offset value missing."));
        return;
      }
    ...
    foreach (PartitionInfo *partition, *partitions) {
      ...
      if (reqPart) {
        if (partitionMap.contains(reqPart)) {
          emit error(
              tr("More than one operating system requires partition number %1")
                  .arg(reqPart));
          return;
        }
        if (reqPart == 1 || reqPart == 5) {
          emit error(
              tr("Operating system cannot require a system partition (1,5)"));
          return;
        }
        if ((reqPart == 2 && partitionMap.contains(4)) ||
            (reqPart == 4 && partitionMap.contains(2))) {
          emit error(tr(
      ...
      if (partition->wantMaximised() ||
          (partition->partitionSizeNominal() * 2048) % PARTITION_ALIGNMENT != 0)
  ...
  /* Assign logical partition numbers to partitions that did not reserve a
   * special number */
  int pnr, bootpnr;
  uint offset = 0;
  ...
  if (_multiDrives) {
    bootpnr = 6;
    offset = getFileContents(sysclassblock(_bootdrive, 5) + "/start")
                 .trimmed()
                 .toUInt() +
             getFileContents(sysclassblock(_bootdrive, 5) + "/size")
                 .trimmed()
                 .toUInt();
  }
  ...
  foreach (OsInfo *image, _images) {
    foreach (PartitionInfo *partition, *(image->partitions())) {
      if (!partition->requiresPartitionNumber()) {
        if (_multiDrives && partition->bootable() &&
            !partition->wantMaximised()) {
  ...
  if (!log_before_prim.isEmpty() &&
      log_before_prim.first()->requiresPartitionNumber() == 2)
    log_before_prim.push_back(log_before_prim.takeFirst());
  if (!log_before_prim.isEmpty() &&
      log_before_prim.first()->requiresPartitionNumber() == 3)
    log_before_prim.push_back(log_before_prim.takeFirst());
  if (!log_before_prim.isEmpty() &&
      log_before_prim.first()->requiresPartitionNumber() == 4)
    log_before_prim.push_back(log_before_prim.takeFirst());
  ...
  foreach (PartitionInfo *p, log_before_prim) {
    ...
    if (p == log_before_prim.last()) {
      /* Let last partition have any remaining space that we couldn't divide
       * evenly */
...
#endif
      if (p->wantMaximised() &&
          (partsizeSectors + PARTITION_GAP) % PARTITION_ALIGNMENT != 0) {
        /* Enlarge partition to close gap to next partition */
        partsizeSectors +=
            PARTITION_ALIGNMENT -
            ((partsizeSectors + PARTITION_GAP) % PARTITION_ALIGNMENT);
      }
    }
  ...
  /* Zero out first sector of partitions, to make sure to get rid of previous
   * file system (label) */
  ...
  foreach (PartitionInfo *p, partitionMap.values()) {
    if (p->partitionSizeSectors())
      QProcess::execute("/bin/dd count=3 bs=512 if=/dev/zero of=" +
                        p->partitionDevice());
  }
  ...
  if (_multiDrives) {
    ...
    if (QProcess::execute("mount -t ext4 " +
                          partdev(_bootdrive, SETTINGS_PARTNR) + " /mnt2") ==
        0) {
...
bool MultiImageWriteThread::writePartitionTable(
    const QString &drive, const QMap<int, PartitionInfo *> &pmap) {
  /* Write partition table using sfdisk */

  /* Fixed NOOBS partition */
  uint startP1 =
      getFileContents(sysclassblock(drive, 1) + "/start").trimmed().toUInt();
  uint sizeP1 =
      getFileContents(sysclassblock(drive, 1) + "/size").trimmed().toUInt();
  /* Fixed start of extended partition. End is not fixed, as it depends on
   * primary partition 3 & 4 */
  int startExtended = startP1 + sizeP1;
  /* Fixed settings partition */
  uint startP5 =
      getFileContents(sysclassblock(drive, SETTINGS_PARTNR) + "/start")
          .trimmed()
          .toUInt();
  uint sizeP5 = getFileContents(sysclassblock(drive, SETTINGS_PARTNR) + "/size")
                    .trimmed()
                    .toUInt();
  ...
  partitionMap.insert(1, new PartitionInfo(1, startP1, sizeP1, "0E",
                                           this)); /* FAT boot partition */
  partitionMap.insert(5, new PartitionInfo(5, startP5, sizeP5, "L",
                                           this)); /* Ext4 settings partition */

  uint sizeExtended = partitionMap.values().last()->endSector() - startExtended;
  if (!partitionMap.contains(2)) {
    partitionMap.insert(
        2, new PartitionInfo(2, startExtended, sizeExtended, "E", this));
  } else {
    /* If an OS already claimed primary partition 2, use out-of-order
     * partitions, and store extended at partition 4 */
    partitionMap.insert(
        4, new PartitionInfo(4, startExtended, sizeExtended, "E", this));
  }
  ...
  for (int i = 1; i <= partitionMap.keys().last(); i++) {
    if (partitionMap.contains(i)) {
      ...
      partitionTable += QByteArray::number(p->offset()) + "," +
                        QByteArray::number(p->partitionSizeSectors()) + "," +
                        p->partitionType();
      if (p->active())
        partitionTable += " *";
      partitionTable += "\n";
  ...
  if (drive == _bootdrive) {
    ...
    QProcess::execute("mount -t ext4 " + partdev(drive, SETTINGS_PARTNR) +
                      " /settings");
  }
  if (QFile::exists("/tmp/media/" + driveP1)) {
    QProcess::execute("mount -o ro -t vfat /dev/" + driveP1 + " /tmp/media/" +
                      driveP1);
  }
...
bool MultiImageWriteThread::processImage(OsInfo *image) {
  ...
  if (QProcess::execute("mount " + partitions->first()->partitionDevice() +
                        " /mnt2") != 0) {
    emit error(tr("%1: Error mounting file system").arg(os_name));
    return false;
  }
  ...
  if (QFile::exists(iconfilename)) {
    if (iconfilename.startsWith("/tmp/media/")) {
      /* Copy icon to settings folder, as USB storage may take longer to get
       * ready on boot */
      QDir dir;
...
QString MultiImageWriteThread::shorten(QString example, int maxLabelLen) {
  ...
  case 3:
    ...
    l2 = rem / 2;
    l1 = rem - l2;
    test = parts.first().left(l1) + "_" + parts.at(1).right(l2) + "_" +
           parts.last().left(r);
    break;
...
QByteArray MultiImageWriteThread::makeLabelUnique(QByteArray label,
                                                  int maxLabelLen) {
  if (label.size() > maxLabelLen) { // restrict to maximum size
    label = shorten(label, maxLabelLen).toAscii();
  }

  if (!isLabelAvailable(label)) {
    if (label.size() == maxLabelLen) { // Make room for extra digit
...
bool MultiImageWriteThread::untar(const QString &tarball) {
  ...
  if (statfs("/mnt2", &st) == 0 && st.f_type == MSDOS_SUPER_MAGIC) {
    /* File system does not support uid/gid, tell bsdtar not to set those or it
     * will error out */
    cmd += " --no-same-owner ";
  }
...
bool MultiImageWriteThread::partclone_restore(const QString &imagePath,
                                              const QString &device) {
  QString cmd = "sh -o pipefail -c \"";
