...
MainWindow::MainWindow(const QString &drive, const QString &defaultDisplay,
    ...
    : QMainWindow(parent), ui(new Ui::MainWindow), _qpd(NULL), _kcpos(0),
      ...
      _showAll(false), _fixate(false), _splash(splash), _settings(NULL),
      _hasWifi(false), _numInstalledOS(0), _devlistcount(0), _netaccess(NULL),
      _displayModeBox(NULL), _drive(drive), _bootdrive(drive) {
  ui->setupUi(this);
  ...
  if (!QFile::exists(settingsPartition)) {
    _qpd = new QProgressDialog(tr("Waiting for SD card (settings partition)"),
                               QString(), 0, 0, this);
    _qpd->setWindowModality(Qt::WindowModal);
    _qpd->setWindowFlags(Qt::Window | Qt::CustomizeWindowHint |
                         Qt::WindowTitleHint);
  ...
  _qpd = new QProgressDialog(tr("Mounting settings partition"), QString(), 0, 0,
                             this);
  _qpd->setWindowModality(Qt::WindowModal);
  _qpd->setWindowFlags(Qt::Window | Qt::CustomizeWindowHint |
                       Qt::WindowTitleHint);
  ...
  if (QProcess::execute("mount -t ext4 " + settingsPartition + " /settings") !=
      0) {
    _qpd->hide();
  ...
  if (QFile::exists("/mnt/os_list_v3.json")) {
    ...
    /* We need a somewhat accurate date for https to work. Normally we retrieve
       that from the repo server, but since we are in testing mode, just set
       date to last modification time of our local file */
    ...
    if (cmdline.length() > pos + searchForLen &&
        cmdline.at(pos + searchForLen) == '"') {
...
void MainWindow::populate() {
  ...
  repopulate();
  _availableMB =
      (getFileContents(sysclassblock(_drive) + "/size").trimmed().toUInt() -
       getFileContents(sysclassblock(_drive, 5) + "/start").trimmed().toUInt() -
...
bool MainWindow::canInstallOs(const QString &name, const QVariantMap &values) {
  /* Can't simply pull "name" from "values" because in some JSON files it's
   * "os_name" and in others it's "name" */
  ...
  if (nameMatchesRiscOS(name)) {
    if (!values.contains(RISCOS_OFFSET_KEY) ||
        (values.value(RISCOS_OFFSET_KEY).toInt() != RISCOS_OFFSET)) {
      return false;
    }
  }

  /* Display OS in list if it is supported or "showall" is specified in
   * recovery.cmdline */
  if (_showAll) {
    return true;
  } else {
...
bool MainWindow::isSupportedOs(const QString &name, const QVariantMap &values) {
  ...
  if (values.contains("supported_models")) {
    QStringList supportedModels =
        values.value("supported_models").toStringList();

    foreach (QString m, supportedModels) {
      /* Check if the full formal model name (e.g. "Raspberry Pi 2 Model B
       * Rev 1.1") contains the string we are told to look for (e.g. "Pi 2") */
...
void MainWindow::on_actionWrite_image_to_disk_triggered() {
  QString warning =
      tr("Warning: this will install the selected Operating System(s). All "
         "existing data on the SD card will be overwritten, including any OSes "
         "that are already installed.");
  if (_drive != "mmcblk0")
    warning.replace(tr("SD card"), tr("drive"));

  if (_silent ||
      QMessageBox::warning(this, tr("Confirm"), warning, QMessageBox::Yes,
    ...
    if (_silent || allSupported ||
        QMessageBox::warning(
            this, tr("Confirm"),
            tr("Warning: incompatible Operating System(s) detected. The "
               "following OSes aren't supported on this revision of Raspberry "
               "Pi and may fail to boot or function correctly:") +
                unsupportedOses,
            QMessageBox::Yes, QMessageBox::No) == QMessageBox::Yes) {
      setEnabled(false);
      ...
      if (_numMetaFilesToDownload == 0) {
        ...
        _qpd =
            new QProgressDialog(tr("The install process will begin shortly."),
                                QString(), 0, 0, this);
        _qpd->setWindowFlags(Qt::Window | Qt::CustomizeWindowHint |
                             Qt::WindowTitleHint);
        _qpd->show();
...
void MainWindow::copyWpa() {
  ...
  /* If user supplied a wpa_supplicant.conf on the FAT partition copy that one
   * to settings regardless */
  if (QFile::exists("/mnt/wpa_supplicant.conf")) {
    qDebug()
        << "Copying user wpa_supplicant.conf to /settings/wpa_supplicant.conf";
    ...
    f.setPermissions(QFile::WriteUser | QFile::ReadGroup | QFile::ReadOther |
                     QFile::ReadUser);

    /* rename the user file to indicate that it has been copied (and prevent it
       being re-copied next time, which could potentially overwrite any SSIDs
       created in the NOOBS GUI) */
    ...
    qDebug()
        << "Copying /etc/wpa_supplicant.conf to /settings/wpa_supplicant.conf";
...
void MainWindow::startNetworking() {
  ...
  proc->start("/sbin/dhcpcd --noarp -e "
  ...
  if (isOnline()) {
    ...
    // QNetworkConfigurationManager *_netconfig = new
    // QNetworkConfigurationManager(this); connect(_netconfig,
    // SIGNAL(onlineStateChanged(bool)), this, SLOT(onOnlineStateChanged(bool)));
    connect(&_networkStatusPollTimer, SIGNAL(timeout()),
            SLOT(pollNetworkStatus()));
...
void MainWindow::downloadListComplete() {
  ...
  if (!dateStr.isEmpty() && QDate::currentDate().year() < 2015) {
    // Qt 4 does not have a standard function for parsing the Date header, but
    // it does have one for parsing a Last-Modified header that uses the same
    // date/time format, so just use that
    QNetworkRequest dummyReq;
    dummyReq.setRawHeader("Last-Modified", dateStr);
    QDateTime parsedDate =
        dummyReq.header(dummyReq.LastModifiedHeader).toDateTime();
...
void MainWindow::processJsonOs(const QString &name, QVariantMap &new_details,
                               QSet<QString> &iconurls) {
  QIcon internetIcon(":/icons/download.png");
  ...
  if (witem) {
    ...
    if ((existing_details["release_date"].toString() <
         new_details["release_date"].toString()) ||
        (existing_details["source"].toString() == SOURCE_INSTALLED_OS)) {
      /* Local version is older (or unavailable). Replace info with newer
       * Internet version */
      new_details.insert("installed",
                         existing_details.value("installed", false));
...
void MainWindow::updateNeeded() {
  ...
  foreach (QListWidgetItem *item, selected) {
    ...
    if (nameMatchesRiscOS(entry.value("name").toString())) {
      ...
      uint startSector = getFileContents(sysclassblock(_drive, 5) + "/start")
                             .trimmed()
                             .toUInt() +
                         getFileContents(sysclassblock(_drive, 5) + "/size")
                             .trimmed()
                             .toUInt();
...
void MainWindow::startImageWrite() {
   ...
   * thread */
  MultiImageWriteThread *imageWriteThread =
      new MultiImageWriteThread(_bootdrive, _drive);
  ...
  connect(imageWriteThread, SIGNAL(newDrive(const QString &)), _qpd,
          SLOT(changeDrive(const QString &)), Qt::BlockingQueuedConnection);
    ...
    if (!isOnline()) {
      ...
      if (ui->list->count() == 0) {
        ...
        if (_hasWifi) {
          QMessageBox::critical(
              this, tr("No network access"),
              tr("Network access is required to use NOOBS without local "
                 "images. Please select your wifi network in the next screen."),
              QMessageBox::Close);
          on_actionWifi_triggered();
        } else {
          QMessageBox::critical(this, tr("No network access"),
                                tr("Wired network access is required to use "
                                   "NOOBS without local images. Please insert "
                                   "a network cable into the network port."),
                                QMessageBox::Close);
        }
      }
    }
...
void MainWindow::pollForNewDisks() {
  ...
  if (list.count() != _devlistcount) {
    foreach (QString devname, list) {
      ...
      if (blocklink.contains("/devices/virtual/") ||
          QFile::exists(blocklink + "/partition"))
        continue;

      QByteArray inflight =
          getFileContents(sysclassblock(devname) + "/inflight").trimmed();
      if (inflight.count() && inflight.left(1) != "0") {
        /* There are currently outstanding IO request, which could mean it
         * hasn't finished reading the partition table. Check again on next
         * round */
        return;
      }

      /* does the drive perhaps have a FAT partition with extra images? */
      if ("/dev/" + devname != _bootdrive &&
      ...
      QString model =
          getFileContents(dirname + "/" + devname + "/device/model").trimmed();
      if (model.isEmpty())
        model =
            getFileContents(dirname + "/" + devname + "/device/name").trimmed();
      QIcon icon;
      ...
      if (ui->targetCombo->findData(devname) == -1) {
        ...
        /* does the partition structure look like a preloaded Pi drive, then
         * select it by default? */
        if (devname == "sda" && QFile::exists(sysclassblock(devname, 1)) &&
            QFile::exists(sysclassblock(devname, 5)) &&
            getFileContents(sysclassblock(devname, 5) + "/size")
                    .trimmed()
                    .toInt() == SETTINGS_PARTITION_SIZE)
...
void MainWindow::on_targetCombo_currentIndexChanged(int index) {
  if (index != -1) {
    ...
    if (devname != "mmcblk0" &&
        (!QFile::exists(sysclassblock(devname, 1)) ||
         ...
         getFileContents(sysclassblock(devname, 5) + "/size")
                 ...
                 .toInt() != SETTINGS_PARTITION_SIZE)) {
      if (QMessageBox::question(
              this, tr("Reformat drive?"),
              tr("Are you sure you want to reformat the drive '%1' for use "
                 "with NOOBS? All existing data on the drive will be deleted!")
                  .arg(devname),
              QMessageBox::Yes, QMessageBox::No) == QMessageBox::Yes) {
    ...
    _availableMB =
        (getFileContents(sysclassblock(_drive) + "/size").trimmed().toUInt() -
         getFileContents(sysclassblock(_drive, 5) + "/start")
             .trimmed()
             .toUInt() -
         getFileContents(sysclassblock(_drive, 5) + "/size")
             .trimmed()
...
void MainWindow::addImagesFromUSB(const QString &device) {
  ...
  dir.mkpath(mntpath);
  if (QProcess::execute("mount -o ro -t vfat /dev/" + device + " " + mntpath) !=
      0) {
    dir.rmdir(mntpath);
    return;
  }
  ...
  foreach (QVariant v, images.values()) {
    ...
    if (witem) {
      ...
      if ((existing_details["release_date"].toString() <=
        ...
        /* Existing item in list is same version or older. Prefer image on USB
         * storage. */
...
void MainWindow::filterList() {
  for (int i = 0; i < ui->list->count(); i++) {
    ...
    if (_drive == "/dev/mmcblk0") {
      ...
      /* If the repo explicity states wheter or not usb is supported use that
       * info */
      if (m.contains(param)) {
        ...
        QString name = m.value("name").toString();
        supportsUsb = (!nameMatchesRiscOS(name) &&
