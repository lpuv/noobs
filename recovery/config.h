...
/* Size of recovery FAT partition in MB when using reformat drive initialization
 * method. */
#define RESCUE_PARTITION_SIZE 63

/* Files that are currently on the FAT partition are normally saved to memory
 * during repartitioning. If files they are larger than number of MB, try
 * resizing the FAT partition instead */
