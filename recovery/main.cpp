...
void showBootMenu(const QString &drive, const QString &defaultPartition,
                  bool setDisplayMode) {
  ...
  ::syscall(SYS_reboot, LINUX_REBOOT_MAGIC1, LINUX_REBOOT_MAGIC2,
            LINUX_REBOOT_CMD_RESTART2, reboot_part.constData());
}

bool hasInstalledOS(const QString &drive) {
  bool installedOsFileExists = false;

  if (QProcess::execute("mount -o ro " + partdev(drive, SETTINGS_PARTNR) +
                        " /settings") == 0) {
...
QString findRecoveryDrive() {
  ...
  foreach (QString devname, list) {
    if (QFile::symLinkTarget("/sys/class/block/" + devname)
            .contains("/devices/virtual/")) { // Skip if partition is virtual.
      continue;
    }

    int partitionNr = 0;
    QRegExp partnrRx("([0-9]+)$");
    if (partnrRx.indexIn(devname) !=
        -1) { // Does devname end in 1 or more digits? Yes->Read it
      partitionNr = partnrRx.cap(1).toInt();

      if (partitionNr > 1) { // Skip if partition Number >1
        continue;
      }
    }

    if (partitionNr == 0) { // Check for a possible non-MBR drive
    ...
    if (QProcess::execute("mount -t vfat -o ro /dev/" + devname + " /mnt") ==
        0) {
...
int main(int argc, char *argv[]) {
  ...
  // Unless we have a touch screen, wait for keyboard to appear before
  // displaying anything
  if (!hasTouchScreen)
  ...
  if (!driveReady) {
    QMessageBox::critical(
        NULL, "Files not found",
        QString("Cannot find the drive with NOOBS files %1").arg(drive),
        QMessageBox::Close);
    return 1;
  }
  qDebug() << "NOOBS drive:" << drive;

  // If -runinstaller is not specified, only continue if SHIFT is pressed, GPIO
  // is triggered, or no OS is installed (/settings/installed_os.json does not
  // exist)
  bool bailout = !runinstaller && !force_trigger &&
  ...
  mw.setGeometry(QStyle::alignedRect(Qt::LeftToRight, Qt::AlignCenter,
                                     mw.size(),
  ...
  ld->setGeometry(
      QStyle::alignedRect(Qt::LeftToRight, Qt::AlignHCenter | Qt::AlignBottom,
                          ld->size(), a.desktop()->availableGeometry()));
  ld->show();
#endif
